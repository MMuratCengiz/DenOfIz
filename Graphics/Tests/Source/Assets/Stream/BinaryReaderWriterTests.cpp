...
TEST( BinarySerdeTest, BasicTypes )
{
    ...
    ASSERT_STREQ( reader.ReadString( ), "Hello Binary!" );
...
{
    BinaryContainer  container;
    DenOfIz_Float2   vec2 = { 1.0f, 2.0f };
    DenOfIz_Float3   vec3 = { 3.0f, 4.0f, 5.0f };
    DenOfIz_Float4   vec4 = { 6.0f, 7.0f, 8.0f, 9.0f };
    DenOfIz_Float4x4 mat4;

    for ( int i = 0; i < 4; ++i )
    {
        for ( int j = 0; j < 4; ++j )
        {
            float *mat4_base       = &mat4._11;
            mat4_base[ i * 4 + j ] = ( i + 1 ) * ( j + 1 );
        }
    }
    ...
    BinaryReader     reader( container );
    DenOfIz_Float2   r_vec2 = reader.ReadFloat_2( );
    DenOfIz_Float3   r_vec3 = reader.ReadFloat_3( );
    DenOfIz_Float4   r_vec4 = reader.ReadFloat_4( );
    DenOfIz_Float4x4 r_mat4 = reader.ReadFloat_4x4( );
    ...
    for ( int i = 0; i < 4; ++i )
    {
        for ( int j = 0; j < 4; ++j )
        {
            const float *r_mat4_base = &r_mat4._11;
            const float *mat4_base   = &mat4._11;

            ASSERT_FLOAT_EQ( r_mat4_base[ i * 4 + j ], mat4_base[ i * 4 + j ] );
        }
    }
}
...
TEST( BinarySerdeTest, InteropArrayByteSupport )
{
    ...
    const DenOfIz_ByteArrayView byteData = container.GetData( );

    BinaryReader reader( byteData );
    ASSERT_EQ( reader.ReadUInt32( ), 12345 );
    ASSERT_FLOAT_EQ( reader.ReadFloat( ), 3.14159f );
    ASSERT_STREQ( reader.ReadString( ), "TestString" );
