...
TextVertexCacheKey UITextVertexCache::CreateTextVertexKey( const Clay_RenderCommand *command, const float effectiveScale, const float adjustedY )
{
    const auto &data   = command->renderData.text;
    const auto &bounds = command->boundingBox;

    TextVertexCacheKey key{ };
    key.textHash       = TextLayoutCache::HashString( DenOfIz_StringView( data.stringContents.chars, data.stringContents.length ) );
    key.fontId         = data.fontId;
    key.fontSize       = data.fontSize > 0 ? static_cast<uint32_t>( data.fontSize ) : 0;
    key.posX           = bounds.x;
    key.posY           = adjustedY;
    key.colorRGBA      = ColorToRGBA( data.textColor );
    key.letterSpacing  = data.letterSpacing;
