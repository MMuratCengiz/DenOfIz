...
XMFLOAT4 ClayColorToFloat4( const Clay_Color &color )
{
    return XMFLOAT4( color.r / 255.0f, color.g / 255.0f, color.b / 255.0f, color.a / 255.0f );
}

void AddVertex( std::vector<UIVertex> *vertices, const float x, const float y, const float z, const float u, const float v, const XMFLOAT4 &color, const uint32_t textureIndex )
{
    UIVertex vertex;
    vertex.Position     = DenOfIz_Float3{ x, y, z };
    vertex.TexCoord     = DenOfIz_Float2{ u, v };
    vertex.Color        = DenOfIz_Float4{ color.x, color.y, color.z, color.w };
    vertex.TextureIndex = textureIndex;

    vertices->push_back( vertex );
}

void AddTriangle( std::vector<uint32_t> *indices, const uint32_t v0, const uint32_t v1, const uint32_t v2 )
{
...
void AddQuad( std::vector<uint32_t> *indices, const uint32_t topLeft, const uint32_t topRight, const uint32_t bottomRight, const uint32_t bottomLeft )
{
