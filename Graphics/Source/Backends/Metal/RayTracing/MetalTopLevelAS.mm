...
static void TransposeTransformMatrix( const DenOfIz_Float4x4 &src, MTLPackedFloat4x3 &dst )
{
    dst.columns[0] = MTLPackedFloat3Make( src._11, src._21, src._31 );
    dst.columns[1] = MTLPackedFloat3Make( src._12, src._22, src._32 );
    dst.columns[2] = MTLPackedFloat3Make( src._13, src._23, src._33 );
    dst.columns[3] = MTLPackedFloat3Make( src._14, src._24, src._34 );
}

MetalTopLevelAS::MetalTopLevelAS( MetalContext *context, const DenOfIz_TopLevelASDesc &desc ) : m_context( context ), m_desc( desc )
{
    ...
    for ( size_t i = 0; i < desc.Instances.NumElements; ++i )
    {
        ...
        const MetalBottomLevelAS     *blas            = dynamic_cast<const MetalBottomLevelAS *>( blasInterface );
        ...
        instance->intersectionFunctionTableOffset                  = blas->GeometryType( ) == DENOFIZ_HIT_GROUP_TYPE_TRIANGLES ? DENOFIZ_METAL_TRIANGLE_INTERSECTION_SHADER : DENOFIZ_METAL_PROCEDURAL_INTERSECTION_SHADER;
        ...
        TransposeTransformMatrix( instanceDesc.Transform, instance->transformationMatrix );
    ...
    if ( m_desc.BuildFlags & DENOFIZ_AS_BUILD_ALLOW_UPDATE_BIT )
    {
        m_descriptor.usage |= MTLAccelerationStructureUsageRefit;
    }
    if ( m_desc.BuildFlags & DENOFIZ_AS_BUILD_PREFER_FAST_BUILD_BIT )
    {
...
void MetalTopLevelAS::UpdateInstanceTransforms( const DenOfIz_UpdateTransformsDesc &desc )
{
    for ( size_t i = 0; i < desc.Transforms.NumElements; ++i )
    {
        MTLAccelerationStructureUserIDInstanceDescriptor *instance = &m_instanceDescriptors[ i ];
        TransposeTransformMatrix( desc.Transforms.Elements[ i ], instance->transformationMatrix );
    }
}

size_t MetalTopLevelAS::BuildNumBytes( ) const
{
