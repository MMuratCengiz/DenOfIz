...
DX12CommandQueue::DX12CommandQueue( DX12Context *context, const DenOfIz_CommandQueueDesc &desc ) : m_context( context ), m_desc( desc )
{
    ...
    {
    case DENOFIZ_QUEUE_PRIORITY_LOW:
        queueDesc.Priority = D3D12_COMMAND_QUEUE_PRIORITY_NORMAL;
        break;
    case DENOFIZ_QUEUE_PRIORITY_NORMAL:
        queueDesc.Priority = D3D12_COMMAND_QUEUE_PRIORITY_HIGH;
        break;
    case DENOFIZ_QUEUE_PRIORITY_HIGH:
...
void DX12CommandQueue::ExecuteCommandLists( const DenOfIz_ExecuteCommandListsDesc &executeCommandListsDesc )
{
    for ( size_t i = 0; i < executeCommandListsDesc.WaitSemaphores.NumElements; i++ )
    {
        const auto *dx12Semaphore = dynamic_cast<DX12Semaphore *>( DENOFIZ_FROM_HANDLE( ISemaphore, executeCommandListsDesc.WaitSemaphores.Elements[ i ] ) );
    ...
    {
        const auto *dx12CmdList = dynamic_cast<DX12CommandList *>( DENOFIZ_FROM_HANDLE( ICommandList, executeCommandListsDesc.CommandLists.Elements[ i ] ) );
    ...
    {
        auto *dx12Fence = dynamic_cast<DX12Fence *>( DENOFIZ_FROM_HANDLE( IFence, executeCommandListsDesc.Signal ) );
        dx12Fence->NotifyCommandQueue( m_commandQueue.get( ) );
    }

    for ( size_t i = 0; i < executeCommandListsDesc.SignalSemaphores.NumElements; i++ )
    {
        auto *dx12Semaphore = dynamic_cast<DX12Semaphore *>( DENOFIZ_FROM_HANDLE( ISemaphore, executeCommandListsDesc.SignalSemaphores.Elements[ i ] ) );
...
DenOfIz_QueueType DX12CommandQueue::GetQueueType( ) const
{
