...
DX12CommandListPool::DX12CommandListPool( DX12Context *context, DenOfIz_CommandListPoolDesc desc ) : m_desc( desc )
{
    DZ_NOT_NULL( context );
    DZ_ASSERTM( desc.NumCommandLists > 0, "CommandListCount must be greater than 0" );

    m_context                            = context;
    const DX12CommandQueue *commandQueue = dynamic_cast<DX12CommandQueue *>( DENOFIZ_FROM_HANDLE( ICommandQueue, desc.CommandQueue ) );

    for ( uint32_t i = 0; i < desc.NumCommandLists; i++ )
    {
        const D3D12_COMMAND_LIST_TYPE commandListType = DenOfIz_DX12EnumConverter_ConvertQueueType( commandQueue->GetQueueType( ) );
    ...
    DenOfIz_CommandListDesc commandListCreateInfo{ };
...
DenOfIz_CommandListArray DX12CommandListPool::GetCommandLists( )
{
