...
DX12LocalRootSignature::DX12LocalRootSignature( DX12Context *context, const DenOfIz_LocalRootSignatureDesc &desc ) : m_context( context ), m_desc( desc )
{
    ...
    for ( uint32_t i = 0; i < desc.ResourceBindings.NumElements; ++i )
    {
        ...
        case DENOFIZ_RESOURCE_BINDING_TYPE_CONSTANT_BUFFER:
            ...
            m_shaderRecordDataNumBytes += binding.NumBytes;
...
bool DX12LocalRootSignature::HasBinding( const DenOfIz_ResourceBindingType type, const uint32_t bindingIndex ) const
{
    for ( uint32_t i = 0; i < m_desc.ResourceBindings.NumElements; ++i )
    {
        const auto &binding = m_desc.ResourceBindings.Elements[ i ];
        if ( DenOfIz_ResourceBindingType_FromDescriptor( binding.Descriptor ) == type && binding.Binding == bindingIndex )
        {
            return true;
        }
    }
