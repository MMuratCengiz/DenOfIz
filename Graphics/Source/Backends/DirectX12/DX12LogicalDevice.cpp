...
void DX12LogicalDevice::CreateDevice( const DenOfIz_LogicalDeviceDesc &desc )
{
    m_deviceDesc = desc;
    ...
    if ( desc.EnableValidationLayers )
    {
        if ( wil::com_ptr<ID3D12Debug> debugController; SUCCEEDED( D3D12GetDebugInterface( IID_PPV_ARGS( debugController.put( ) ) ) ) )
        {
            ...
            spdlog::info( "Direct3D Debug Layer enabled" );
        }
        else
        {
...
DenOfIz_PhysicalDeviceArray DX12LogicalDevice::ListPhysicalDevices( )
{
...
void DX12LogicalDevice::CreateDeviceInfo( IDXGIAdapter1 &adapter, DenOfIz_PhysicalDevice &physicalDevice )
{
    DXGI_ADAPTER_DESC adapterDesc;
    DX_CHECK_RESULT( adapter.GetDesc( &adapterDesc ) );
    physicalDevice.Id = adapterDesc.DeviceId;
    const std::wstring adapterName( adapterDesc.Description );
    m_deviceNameStorage.emplace_back( adapterName.begin( ), adapterName.end( ) );
...
void DX12LogicalDevice::LoadPhysicalDevice( const DenOfIz_PhysicalDevice &device )
{
    ...
    if ( m_deviceDesc.EnableValidationLayers )
    {
        if ( const wil::com_ptr<ID3D12InfoQueue1> d3dInfoQueue = m_context->D3DDevice.query<ID3D12InfoQueue1>( ) )
        {
            ...
            for ( auto messageId : hide )
            {
                d3dInfoQueue->SetBreakOnID( messageId, false );
            }
            d3dInfoQueue->SetBreakOnID( D3D12_MESSAGE_ID_DESCRIPTOR_HEAP_NOT_SHADER_VISIBLE, false );
    ...
    m_context->IndirectSignatureCache = std::make_unique<DX12IndirectSignatureCache>( m_context->D3DDevice.get( ) );
...
ICommandQueue *DX12LogicalDevice::CreateCommandQueue( const DenOfIz_CommandQueueDesc &desc )
{
    return new DX12CommandQueue( m_context.get( ), desc );
}

ICommandListPool *DX12LogicalDevice::CreateCommandListPool( const DenOfIz_CommandListPoolDesc &poolDesc )
{
    return new DX12CommandListPool( m_context.get( ), poolDesc );
}

IPipeline *DX12LogicalDevice::CreatePipeline( const DenOfIz_PipelineDesc &pipelineDesc )
{
    return new DX12Pipeline( m_context.get( ), pipelineDesc );
}

IPipelineCache *DX12LogicalDevice::CreatePipelineCache( const DenOfIz_PipelineCacheDesc &desc )
{
    return new DX12PipelineCache( m_context.get( ), desc );
}

ISwapChain *DX12LogicalDevice::CreateSwapChain( const DenOfIz_SwapChainDesc &swapChainDesc )
{
    return new DX12SwapChain( m_context.get( ), swapChainDesc );
}

IRootSignature *DX12LogicalDevice::CreateRootSignature( const DenOfIz_RootSignatureDesc &rootSignatureDesc )
{
    return new DX12RootSignature( m_context.get( ), rootSignatureDesc );
}

IInputLayout *DX12LogicalDevice::CreateInputLayout( const DenOfIz_InputLayoutDesc &inputLayoutDesc )
{
    return new DX12InputLayout( inputLayoutDesc );
}

IBindGroupLayout *DX12LogicalDevice::CreateBindGroupLayout( const DenOfIz_BindGroupLayoutDesc &desc )
{
    return new DX12BindGroupLayout( m_context.get( ), desc );
}

IBindGroup *DX12LogicalDevice::CreateBindGroup( const DenOfIz_BindGroupDesc &descriptorTableDesc )
{
    return new DX12BindGroup( m_context.get( ), descriptorTableDesc );
}
...
IBuffer *DX12LogicalDevice::CreateBuffer( const DenOfIz_BufferDesc &bufferDesc )
{
    return new DX12Buffer( m_context.get( ), bufferDesc );
}

ITexture *DX12LogicalDevice::CreateTexture( const DenOfIz_TextureDesc &textureDesc )
{
    return new DX12Texture( m_context.get( ), textureDesc );
}

ISampler *DX12LogicalDevice::CreateSampler( const DenOfIz_SamplerDesc &samplerDesc )
{
    return new DX12Sampler( m_context.get( ), samplerDesc );
}

IQueryPool *DX12LogicalDevice::CreateQueryPool( const DenOfIz_QueryPoolDesc &desc )
{
    return new DX12QueryPool( m_context.get( ), desc );
}

ITopLevelAS *DX12LogicalDevice::CreateTopLevelAS( const DenOfIz_TopLevelASDesc &createDesc )
{
    return new DX12TopLevelAS( m_context.get( ), createDesc );
}

IBottomLevelAS *DX12LogicalDevice::CreateBottomLevelAS( const DenOfIz_BottomLevelASDesc &createDesc )
{
    return new DX12BottomLevelAS( m_context.get( ), createDesc );
}

IShaderBindingTable *DX12LogicalDevice::CreateShaderBindingTable( const DenOfIz_ShaderBindingTableDesc &createDesc )
{
    return new DX12ShaderBindingTable( m_context.get( ), createDesc );
}

ILocalRootSignature *DX12LogicalDevice::CreateLocalRootSignature( const DenOfIz_LocalRootSignatureDesc &createDesc )
{
    return new DX12LocalRootSignature( m_context.get( ), createDesc );
}

IShaderLocalData *DX12LogicalDevice::CreateShaderLocalData( const DenOfIz_ShaderLocalDataDesc &createDesc )
{
