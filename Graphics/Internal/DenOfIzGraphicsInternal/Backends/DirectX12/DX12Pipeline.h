...
namespace DenOfIz
{
    ...
    public:
        DX12Pipeline( DX12Context *context, DenOfIz_PipelineDesc desc );
    ...
    private:
        void                  SetMSAASampleCount( const DenOfIz_PipelineDesc &desc, D3D12_GRAPHICS_PIPELINE_STATE_DESC &psoDesc ) const;
        void                  SetGraphicsShaders( D3D12_GRAPHICS_PIPELINE_STATE_DESC &psoDesc ) const;
        D3D12_SHADER_BYTECODE GetShaderByteCode( const DenOfIz_CompiledShaderStage *const &compiledShader ) const;
        void                  CreateGraphicsPipeline( );
        void                  CreateComputePipeline( );
        void                  CreateRayTracingPipeline( );
        void                  CreateMeshPipeline( );
        void                  PrintRayTracingPipelineDesc( const D3D12_STATE_OBJECT_DESC *desc ) const;
        void                  InitStencilFace( D3D12_DEPTH_STENCILOP_DESC &stencilFace, const DenOfIz_StencilFace &face ) const;
