...
shaderStages.NumElements = 2;
shaderStages.Elements = new DenOfIz_ShaderStageDesc[2];

// Add vertex shader
DenOfIz_ShaderStageDesc &vertexShaderDesc = shaderStages.Elements[0];
vertexShaderDesc.Path = InteropString("Assets/Shaders/FullscreenQuad.vs.hlsl");
vertexShaderDesc.Stage = DENOFIZ_SHADER_STAGE_VERTEX_BIT;

// Add pixel shader
DenOfIz_ShaderStageDesc &pixelShaderDesc = shaderStages.Elements[1];
pixelShaderDesc.Path = InteropString("Assets/Shaders/SampleBasic.ps.hlsl");
...
DenOfIz_ResourceBindGroupDesc bindGroupDesc;
...
DenOfIz_ResourceBindGroupDesc bindGroupDesc = RootConstantBindGroupDesc(rootSignature.get());
...
For more complex scenarios, such as binding parts of a buffer or a buffer array, you can use the `DenOfIz_BindBufferDesc` struct:
...
bufferDesc.Descriptor = BitSet(DENOFIZ_RESOURCE_DESCRIPTOR_UNIFORM_BUFFER_BIT);
bufferDesc.HeapType = DENOFIZ_HEAP_TYPE_CPU_GPU;
std::unique_ptr<IBuffer> buffer = std::unique_ptr<IBuffer>(device->CreateBuffer(bufferDesc));
...
DenOfIz_ShaderStageDesc &pixelShaderDesc = shaderStages.Elements[1];
pixelShaderDesc.Path = InteropString("Assets/Shaders/SampleTexture.ps.hlsl");
pixelShaderDesc.Stage = DENOFIZ_SHADER_STAGE_PIXEL_BIT;
...
DenOfIz_BufferDesc constantBufferDesc;
constantBufferDesc.NumBytes = sizeof(Constants);
constantBufferDesc.Descriptor = BitSet(DENOFIZ_RESOURCE_DESCRIPTOR_UNIFORM_BUFFER_BIT);
constantBufferDesc.HeapType = DENOFIZ_HEAP_TYPE_CPU_GPU;
std::unique_ptr<IBuffer> constantBuffer = std::unique_ptr<IBuffer>(device->CreateBuffer(constantBufferDesc));
...
std::unique_ptr<ITexture> texture = std::unique_ptr<ITexture>(device->CreateTexture(textureDesc));

// Create sampler
DenOfIz_SamplerDesc samplerDesc;
samplerDesc.Filter = DenOfIz_Filter::Linear;
...
DenOfIz_ResourceBindGroupDesc viewProjBindGroupDesc;
...
DenOfIz_ResourceBindGroupDesc modelBindGroupDesc;
