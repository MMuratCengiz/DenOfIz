...
void VulkanImage::Dispose(VulkanContext* context)
{
...
void VulkanImage::Create(VulkanContext* context, const VulkanImageCreateInfo& createInfo)
{
	...
	vmaCreateImage(context->Vma, (VkImageCreateInfo*)&imageCreateInfo, &allocationCreateInfo, (VkImage*)&Instance, &Allocation, nullptr);
...
VulkanSamplerResource::VulkanSamplerResource(DenOfIz::VulkanContext* context, const DenOfIz::SamplerCreateInfo& createInfo)
...
void VulkanSamplerResource::Allocate(const void* newImage)
{
	...
	mipLevels = (uint32_t)std::floor(std::log2(std::max(createInfo.Width, createInfo.Height))) + 1;
	...
	vk::Format format = VulkanEnumConverter::ConvertImageFormat(createInfo.Format);
	...
	vmaCreateImage(context->Vma, (VkImageCreateInfo*)&imageCreateInfo, &allocationCreateInfo, (VkImage*)&(image.Instance), &(image.Allocation), nullptr);

	vk::ImageViewCreateInfo imageViewCreateInfo{};

	imageViewCreateInfo.image = image.Instance;
	imageViewCreateInfo.viewType = vk::ImageViewType::e2D;
	...
	image.ImageView = context->LogicalDevice.createImageView(imageViewCreateInfo);
	image.Sampler = context->LogicalDevice.createSampler(GetSamplerCreateInfo());
	...
	VulkanUtilities::InitStagingBuffer(context, stagingBuffer, stagingAllocation, newImage, createInfo.Width * createInfo.Height * 4);
	VulkanUtilities::RunOneTimeCommand(context, [&](vk::CommandBuffer& commandBuffer)
	{
		...
		memoryBarrier.image = image.Instance;
		...
		bufferImageCopy.imageExtent = vk::Extent3D{ createInfo.Width, createInfo.Height, 1 };

		commandBuffer.copyBufferToImage(stagingBuffer, image.Instance, vk::ImageLayout::eTransferDstOptimal, 1, &bufferImageCopy);
	});

	vmaDestroyBuffer(context->Vma, stagingBuffer, stagingAllocation);
...
void VulkanSamplerResource::GenerateMipMaps()
{
	int32_t mipWidth = createInfo.Width, mipHeight = createInfo.Height;
	...
	VulkanUtilities::RunOneTimeCommand(context, [&](vk::CommandBuffer& commandBuffer)
	{
		...
		for (uint32_t index = 1; index < mipLevels; ++index)
		{
			...
			toShaderFormat.image = image.Instance;
		...
		finalFormat.image = image.Instance;
		finalFormat.subresourceRange.aspectMask = vk::ImageAspectFlagBits::eColor;
		finalFormat.subresourceRange.baseMipLevel = mipLevels - 1;
...
vk::SamplerCreateInfo VulkanSamplerResource::GetSamplerCreateInfo()
{
	vk::SamplerCreateInfo samplerCreateInfo{};

	samplerCreateInfo.magFilter = VulkanEnumConverter::ConvertFilter(createInfo.MagFilter);
	samplerCreateInfo.minFilter = VulkanEnumConverter::ConvertFilter(createInfo.MinFilter);
	samplerCreateInfo.addressModeU = VulkanEnumConverter::ConvertAddressMode(createInfo.AddressModeU);
	samplerCreateInfo.addressModeV = VulkanEnumConverter::ConvertAddressMode(createInfo.AddressModeV);
	samplerCreateInfo.addressModeW = VulkanEnumConverter::ConvertAddressMode(createInfo.AddressModeW);
	...
	samplerCreateInfo.compareOp = VulkanEnumConverter::ConvertCompareOp(createInfo.CompareOp);
	samplerCreateInfo.mipmapMode = VulkanEnumConverter::ConvertMipmapMode(createInfo.MipmapMode);
