...
void DX12SwapChain::CreateSwapChain()
{
	...
	DX_CHECK_RESULT(m_context->D3DDevice->CreateCommittedResource(
			&depthHeapProperties,
...
void DX12SwapChain::Resize(uint32_t width, uint32_t height)
{
	...
	HRESULT hr = m_swapChain->ResizeBuffers(
			...
			width,
			height,
			DX12EnumConverter::ConvertImageFormat(m_swapChainCreateInfo.BackBufferFormat),
...
IImageResource* DX12SwapChain::GetRenderTarget(uint32_t frame)
{
	return m_renderTargets[frame].get();
}

ImageFormat DX12SwapChain::GetPreferredFormat()
{
