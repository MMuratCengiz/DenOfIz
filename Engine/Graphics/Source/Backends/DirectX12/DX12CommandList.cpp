...
DX12CommandList::DX12CommandList(DX12Context* context, CommandListCreateInfo createInfo)
		:m_context(context), m_createInfo(createInfo)
{
	...
	m_context->D3DDevice->CreateCommandList(
			0,
			commandListType,
...
{
	m_commandList->RSSetDepthBias(constantFactor, clamp, slopeFactor);
}
...
void DX12CommandList::SetPipelineBarrier(const PipelineBarrier& barrier)
{
	...
	for (const ImageBarrierInfo& imageBarrier : barrier.GetImageBarriers())
	{
		...
		D3D12_RESOURCE_BARRIER resourceBarrier = CD3DX12_RESOURCE_BARRIER::Transition(pResource,
				DX12EnumConverter::ConvertResourceState(imageBarrier.OldState),
	...
	for (const BufferBarrierInfo& bufferBarrier : barrier.GetBufferBarriers())
	{
		...
		D3D12_RESOURCE_BARRIER resourceBarrier = CD3DX12_RESOURCE_BARRIER::Transition(pResource,
				DX12EnumConverter::ConvertResourceState(bufferBarrier.OldState),
