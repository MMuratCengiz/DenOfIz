...
SpvProgram::SpvProgram(std::vector<CompiledShader> shaderInfos) :Shaders(std::move(shaderInfos))
{
	bool first = true;
...
void SpvProgram::CreateUniformInput(const spirv_cross::Compiler& compiler, const UniformType& uniformType, const spirv_cross::Resource resource,
		const ShaderStage stage)
{
	SpvDecoration decoration = GetDecoration(compiler, resource);
	if (decoration.Name == "type.$Globals") { // Todo recursively read all children instead?
...
ShaderVarType SpvProgram::SpvTypeToCustomType(const spirv_cross::SPIRType& type)
{
	...
	{
		if (numOfElements == 1) return ImageFormat::R32Sint;
		if (numOfElements == 2) return ImageFormat::R32G32Sint;
		if (numOfElements == 3) return ImageFormat::R32G32B32Sint;
		if (numOfElements == 4) return ImageFormat::R32G32B32A32Sint;
	...
	{
		if (numOfElements == 1) return ImageFormat::R64Sint;
		if (numOfElements == 2) return ImageFormat::R64G64Sint;
		if (numOfElements == 3) return ImageFormat::R64G64B64Sint;
		if (numOfElements == 4) return ImageFormat::R64G64B64A64Sint;
		return ImageFormat::Undefined;
	};
#endif
	...
	{
		if (numOfElements == 1) return ImageFormat::R32Uint;
		if (numOfElements == 2) return ImageFormat::R32G32Uint;
		if (numOfElements == 3) return ImageFormat::R32G32B32Uint;
		if (numOfElements == 4) return ImageFormat::R32G32B32A32Uint;
		return ImageFormat::Undefined;
	};

	auto make64UInt = [](const uint32_t& numOfElements) -> ImageFormat
	{
		if (numOfElements == 1) return ImageFormat::R64Uint;
		if (numOfElements == 2) return ImageFormat::R64G64Uint;
		if (numOfElements == 3) return ImageFormat::R64G64B64Uint;
		if (numOfElements == 4) return ImageFormat::R64G64B64A64Uint;
		return ImageFormat::Undefined;
	};

	auto make32Float = [](const uint32_t& numOfElements) -> ImageFormat
	{
		if (numOfElements == 1) return ImageFormat::R32Sfloat;
		if (numOfElements == 2) return ImageFormat::R32G32Sfloat;
		if (numOfElements == 3) return ImageFormat::R32G32B32Sfloat;
		if (numOfElements == 4) return ImageFormat::R32G32B32A32Sfloat;
		return ImageFormat::Undefined;
	};

	auto make64Float = [](const uint32_t& numOfElements) -> ImageFormat
	{
		if (numOfElements == 1) return ImageFormat::R64Sfloat;
		if (numOfElements == 2) return ImageFormat::R64G64Sfloat;
		if (numOfElements == 3) return ImageFormat::R64G64B64Sfloat;
		if (numOfElements == 4) return ImageFormat::R64G64B64A64Sfloat;
		return ImageFormat::Undefined;
	};
...
SpvDecoration SpvProgram::GetDecoration(const spirv_cross::Compiler& compiler, const spirv_cross::Resource& resource)
{
	...
	{
		uint32_t structSize = (uint32_t) compiler.get_declared_struct_size(decoration.Type);
		...
		{
			uint32_t size = (uint32_t) compiler.get_declared_struct_member_size(decoration.Type, i);
