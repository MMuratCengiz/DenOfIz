...
ShaderReflection::ShaderReflection(std::vector<CompiledShader> shaderInfos)
		:
...
void ShaderReflection::OnEachShader(const CompiledShader& shaderInfo, const bool& first)
{
	...
	if (first)
	{
		std::sort(stageInputs.begin(), stageInputs.end(), [&](const spirv_cross::Resource& r1, const spirv_cross::Resource& r2) -> bool
		{
...
void
ShaderReflection::CreateVertexInput(const uint32_t& offset, const ShaderVarType& type, const SpvDecoration& decoration, const uint32_t& size)
{
...
SpvDecoration ShaderReflection::GetDecoration(const spirv_cross::Compiler& compiler, const spirv_cross::Resource& resource)
{
	...
	if (decoration.Type.basetype == spirv_cross::SPIRType::Struct)
	{
		...
		for (uint32_t i = 0; offsetIter != structSize; i++)
		{
			...
			auto& child = decoration.Children.emplace_back(SpvDecoration{ .Type = compiler.get_type(decoration.Type.member_types[i]), .Set = decoration.Set, });
