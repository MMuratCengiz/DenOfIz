...
{
    IDxcBlob             *blob = shaderInfo.Blob;
    std::vector<uint32_t> codeToUVec(static_cast<uint32_t *>(blob->GetBufferPointer()),
                                     static_cast<uint32_t *>(blob->GetBufferPointer()) + blob->GetBufferSize() / sizeof(uint32_t));
