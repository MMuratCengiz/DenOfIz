...
void BatchResourceCopyHelper::Begin()
{
    m_batchCopy->Begin();
    m_syncCommandList->Begin();
}

UniformBufferHolder BatchResourceCopyHelper::CreateUniformBuffer(std::string name, void *data, uint32_t numBytes)
{
    ...
    auto buffer = m_device->CreateBufferResource(name, bufferDesc);
...
VertexIndexBufferPairHolder BatchResourceCopyHelper::CreateGeometryBuffers(const GeometryData &geometryData)
{
    ...
    PipelineBarrierDesc barrierDesc = PipelineBarrierDesc{}
...
SamplerHolder BatchResourceCopyHelper::CreateSampler(const std::string &name, const SamplerDesc &desc)
{
    return SamplerHolder{ .Sampler = m_device->CreateSampler(name, SamplerDesc{}) };
}

TextureHolder BatchResourceCopyHelper::CreateTexture(const std::string &name, const std::string &path)
{
...
void BatchResourceCopyHelper::Submit()
{
    std::unique_ptr<ISemaphore> batchCopyWait = m_device->CreateSemaphore();
    m_batchCopy->End(batchCopyWait.get());

    std::unique_ptr<ISemaphore> syncWait = m_device->CreateSemaphore();
    ExecuteDesc                 executeDesc{};
    executeDesc.WaitOnSemaphores.push_back(batchCopyWait.get());
    executeDesc.NotifySemaphores.push_back(syncWait.get());
