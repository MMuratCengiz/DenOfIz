...
void BasicCompute( const GraphicsApi& gApi )
{
    ...
    std::unique_ptr<IBufferResource> buffer = logicalDevice->CreateBufferResource( "buffer", bufferDesc );
    ...
    resourceBindGroup->Update( UpdateDesc{ }.Buffer( "computeReadBack", buffer.get( ) ) );
    ...
    auto readBack           = logicalDevice->CreateBufferResource( "readBack", bufferDesc );
